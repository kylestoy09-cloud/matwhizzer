-- Rewrite school_wrestlers RPC to include wrestlers from in-season sources.
--
-- Previously only queried tournament_entries (postseason: districts/regions/state).
-- Now unions three sources:
--   1. tournament_entries  — postseason wrestlers with full placement data
--   2. tournament_bouts    — RTF-imported in-season tournament wrestlers
--   3. dual_meet_matches   — dual-meet wrestlers
--
-- A wrestler who appears in postseason keeps their placement data.
-- A wrestler who only appears in in-season sources gets NULL placements
-- but shows on the roster so they're visible and searchable.
--
-- girls_school_wrestlers is NOT changed here — it is boys-only data for now.
...
-- ── 1. Postseason (existing logic, unchanged) ─────────────────────────────
school_entries AS (
  SELECT
    te.id AS entry_id,
    te.wrestler_id,
...
rtf_wrestlers AS (
  SELECT DISTINCT
    CASE WHEN tb.wrestler1_school_id = p_school_id
         THEN tb.nj_wrestler1_id
         ELSE tb.nj_wrestler2_id
    END                   AS wrestler_id,
  ...
  WHERE ist.season = sl.val
    AND (
      (tb.wrestler1_school_id = p_school_id AND tb.nj_wrestler1_id IS NOT NULL)
      OR
      (tb.wrestler2_school_id = p_school_id AND tb.nj_wrestler2_id IS NOT NULL)
    )
),
...
-- ── Rollback ───────────────────────────────────────────────────────────────
-- Re-run the original CREATE OR REPLACE from create_school_rpcs.py +
-- add_season_to_rpcs.py to restore the postseason-only version.
