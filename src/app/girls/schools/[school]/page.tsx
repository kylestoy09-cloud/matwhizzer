    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="border border-slate-200 rounded-none bg-white px-6 py-12 text-center">
          <p className="text-slate-500 text-sm">No girls wrestling program at this school.</p>
          <Link
            href={`/boys/schools/${schoolParam}`}
                ...
                <ul>
                  {sec.wrestlers.map(w => (
                    <li key={w.wrestler_id} className="flex items-center gap-3 px-4 py-2.5 border-t border-slate-50 first:border-0">
                      ...
                      <Link href={`/wrestler/${w.wrestler_id}`} className="font-medium text-slate-800 hover:text-rose-700 hover:underline">
                        {w.wrestler_name}
                      </Link>
                    </li>
                  ))}
