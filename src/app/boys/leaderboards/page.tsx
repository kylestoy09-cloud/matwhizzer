...
async function fetchPoolStats(pool: string, season: number): Promise<PoolData> {
  const p = { p_gender: 'M' as const, p_pool: pool, p_season: season }
  const [
    { data: fastestPin },
    { data: fastestTf },
    { data: mostPins },
...
async function fetchAnalytics(season: number): Promise<AnalyticsData> {
  const [
    ...
    supabase.rpc('lb_lowest_seeded_champs',   { p_gender: 'M', p_season: season }),
    supabase.rpc('lb_revenge_matches',        { p_gender: 'M', p_season: season }),
  ])
  return {
...
const POOL_OPTIONS = [
  { key: 'district', label: 'Districts',              desc: 'District matches only' },
...
export default async function BoysLeaderboardsPage({
  searchParams,
}: {
  ...
  const [data, logos] = await Promise.all([
    isAnalytics ? fetchAnalytics(season) : fetchPoolStats(activePool, season),
    getSchoolLogos(),
  ])
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="mb-6 text-center">
        <PageHeader title="Boys Leaderboards" />
      ...
      {!isAnalytics ? (
        <>
          ...
          <WrestlerTab d={data as PoolData} poolLabel={poolLabel} logos={logos} />
        </>
      ) : (
        <AnalyticsTab d={data as AnalyticsData} season={season} logos={logos} />
      )}
    </div>
  )
}
...
function WrestlerTab({ d, poolLabel, logos }: { d: PoolData; poolLabel: string; logos: LogoMap }) {
  return (
            ...
            cols={[
              {
                ...
                render: r => <PWCell id={r.wrestler_id} name={r.wrestler_name} school={r.school_name || r.school} {...logos.badgeByIdOrName(r.school_id, r.school_name || r.school)} />,
              },
              {
      ...
      <section>
        ...
        <div className="space-y-8">
          ...
          <LeaderTable<PoolWrestlerPtsRow>
            ...
            description="NJSIAA formula — round base + FALL+2, TF+1.5, MD+1 + placement bonus (district only)"
            rows={d.wrestlerPts}
            cols={[
              {
                label: 'Wrestler', align: 'left',
          ...
          <LeaderTable<PoolTeamRow>
            title="Team Points"
            description="NJSIAA formula — round base + win bonus (FALL+2, TF+1.5, MD+1) + placement bonus"
            rows={d.teamPoints}
            cols={[
              {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <LeaderTable<PoolDomRow>
              title="Hammer Rating"
              description="Avg dominance per match: pins/TFs graded by speed (max 9), MD=2, Dec=1, losses scored inverse"
              rows={d.poolDom}
              cols={[
                {
                  label: 'Wrestler', align: 'left',
...
function AnalyticsTab({ d, season, logos }: { d: AnalyticsData; season: number; logos: LogoMap }) {
    ...
    <div className="space-y-10">
          ...
          cols={[
            {
              ...
              render: r => <WrestlerCell id={r.wrestler_id} name={r.wrestler_name} school={r.school_name || r.school} {...logos.badgeByIdOrName(r.school_id, r.school_name || r.school)} />,
            },
            {
              label: 'Wt', align: 'right',
              render: r => <span className="text-slate-500 tabular-nums">{r.weight}</span>,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <LeaderTable<DistrictRow>
          title="District Strength"
          description="Districts ranked by state qualifiers"
          rows={d.districts}
          cols={[
            {
        ...
        <LeaderTable<SchoolRow>
          title="School Depth"
          description="Schools with the most weight classes having a district winner"
          rows={d.schools}
          cols={[
            {
              label: 'School', align: 'left',
