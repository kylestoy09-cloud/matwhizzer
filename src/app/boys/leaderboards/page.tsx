...
async function fetchPoolStats(pool: string): Promise<PoolData> {
  const p = { p_gender: 'M' as const, p_pool: pool }
  const [
    { data: fastestPin },
    { data: fastestTf },
    { data: mostPins },
...
async function fetchAnalytics(): Promise<AnalyticsData> {
  const [
    ...
    supabase.rpc('lb_dominance',              { p_gender: 'M' }),
    supabase.rpc('lb_district_strength',      { p_gender: 'M' }),
    supabase.rpc('lb_school_depth',           { p_gender: 'M' }),
    supabase.rpc('lb_weight_competitiveness', { p_gender: 'M' }),
    supabase.rpc('lb_comebacks',              { p_gender: 'M' }),
  ])
  return {
